...
import com.fastcampus.projectboard.TestSecurityConfig;
...
@Import({TestSecurityConfig.class, FormDataEncoder.class})   // 내가 직접 만들었기때문에 import해주어야함.
...
//    @Disabled("구현 중")
    ...
    @WithUserDetails(value = "unoTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)   // @WithMockUser 와 달리 인증후 구체적데이터를 준다. unoTest는 아페 TestSecurityConfig에서 지정.
    @DisplayName("[view][POST] 새 게시글 등록 - 정상 호출")                                     // TestSecurityConfig안에 securitySetUp()가 @BeforeTestMethod로 인해서 먼저수행되고, 거기서, 꺼내온다.
    @Test
